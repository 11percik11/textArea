...
//   const {isOpenIndexPopup = OpenPopupId.link.isOpen

//   useEffect(() => {
//     if (!content) return;
//     if (isOpenIndexPopup.link == null || cellIndex == null) return;
...
const UserTable = ({ onCellInfoOpen, content }: Props) => {
  const isOpenIndexPopup = OpenPopupId.link

  useEffect(() => {
    //  console.log("isOpenIndexPopup111", isOpenIndexPopup);
    if (!content) return;
    if (isOpenIndexPopup.index[0] == null || isOpenIndexPopup.index[1] == null) return;
    const row = content.rows[isOpenIndexPopup.index[0]];
    const cell = row?.cells?.[isOpenIndexPopup.index[1]];
    

    if (cell) onCellInfoOpen(cell);
  }, [content, isOpenIndexPopup.index[0], isOpenIndexPopup.index[1], onCellInfoOpen]);
