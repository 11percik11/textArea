  ...
  return (
    <div className="animate-appear w-full h-full bg-[#00000099] backdrop-blur-[40px] fixed z-100">
      ...
      <input
        ...
        type="password"
        disabled={true}
        className="text-center w-full text-white text-[62px]"
      ...
      <div className="mx-auto mt-[64px] w-[452px] h-[384px] grid grid-rows-4 grid-cols-3 gap-[16px]">
        {numbers.map((num, index: number) => (
          <div
            key={index}
            onClick={() => {
              setEnteredPassword((prev) => prev + num.toString());
            }}
            className="flex active:bg-[#75757533] items-center justify-center leading-[100%] font-semibold text-white text-[32px] text-center w-[140px] h-[84px] rounded-[24px] bg-[#FFFFFF33]"
          >
            {num}
          </div>
        ))}
      ...
      <button
        onClick={onBack}
        className="mx-auto mt-[32px] w-[452px] h-[72px] bg-white rounded-[24px] flex items-center justify-center text-[24px] text-accent font-semibold leading-[100%]"
      >
        Назад
      </button>
    </div>
