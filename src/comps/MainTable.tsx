  ...
  return (
    <div className="w-[1856px] border-[2px] border-stroke rounded-[24px] overflow-auto">
      {content.rows.map((row, index: number) => (
        <div key={index} className={`flex ${colors[index % 4]}`}>
          <div
            className={`bg-[#0000001A] min-w-[232px] max-w-[232px] px-[24px] py-[40px] text-[20px] text-text font-bold leading-[100%] text-center`}
          >
            {testTitles[index]}
          </div>
          {row.content.map((cell: Cell, cellIndex: number) => (
            <div
              key={cellIndex}
              className={`${colors[index % 4]} border-[1px] border-stroke p-[24px] w-[406px] flex justify-center items-center`}
            >
              {!!cell && (
