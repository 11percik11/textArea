...
const MainCell = ({data, onOpen}: Props) => {
    return(
        <div 
            ...
            >
                <img src={data.media[0].url} alt="media" className={`object-cover w-full h-full ${data.isTitleHidden && "blur-[26px] absolute"}`}/>
                <div hidden={!data.isTitleHidden} className="bg-[#00000066] w-[334px] h-[188px] absolute"/>
                <img hidden={!data.isTitleHidden} src={data.media[0].url} alt="media" className="object-fit h-full mx-auto left-0 right-0 absolute" />
            </div>
            }
            <div className="text-center">
                {!data.isTitleHidden && data.title}
                <div className="text-[24px]">{data.type==="timeline" && data.value}</div>
            </div>
        </div>
    );
};
